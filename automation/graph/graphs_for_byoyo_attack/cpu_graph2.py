...
# Em vez de aceitar um link do OpenCost API, ele aceita um ficheiro .json com exatamente a mesma informacao da API. 
# A diferenca e que agora se passar mt tempo, com o fichiero os dados n mudam.

# Load data from a local JSON file instead of fetching from a URL
with open('logs_yoyo-attack.json', 'r') as f:
    data = json.load(f)
